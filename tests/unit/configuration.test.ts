...
it("updates group enablement without changing identity and keeps fallback enabled", () => {
  ...
  const updated = updateManagedGroup(withGroup, withGroup.groups[1]!.id, {
    enabled: false
  });
  expect(updated.groups[1]?.enabled).toBe(false);
  expect(() =>
    updateManagedGroup(updated, updated.fallbackGroupId, { enabled: false })
  ).toThrow(/fallback/);
...
it("accepts a non-allow global duplicate policy without bumping schemaVersion", () => {
  ...
  expect(
    validateConfiguration(withDomain).duplicateSettings.globalPolicy.kind
  ).toBe("domain");
...
it("accepts integer pause timestamps and rejects invalid numeric pause values", () => {
  ...
  expect(() =>
    validateConfiguration({ ...configuration, globalPausedUntil: 1.5 })
  ).toThrow();
  expect(() =>
    validateConfiguration({ ...configuration, globalPausedUntil: -1 })
  ).toThrow();
});
