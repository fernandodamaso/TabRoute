...
describe("production build scan", () => {
  it("accepts the exact Chrome production manifest", async () => {
    const result = await scanProductionBuild("C:/build", {
      readManifest: async () => JSON.stringify({ manifest_version: 3, incognito: "not_allowed", permissions: ["tabs", "tabGroups", "storage", "sessions", "alarms"] }),
      listFiles: async () => [],
  ...
  it.each([
    ...
    ["duplicate", ["tabs", "tabs", "tabGroups", "storage", "sessions", "alarms"]],
    ["wrong order", ["storage", "tabs", "tabGroups", "sessions", "alarms"]]
  ...
  it("recursively rejects nested workbench paths and marker bytes without rejecting binary assets", async () => {
    ...
    const result = await scanProductionBuild("C:/build", {
      readManifest: async () => JSON.stringify({ manifest_version: 3, incognito: "not_allowed", permissions: ["tabs", "tabGroups", "storage", "sessions", "alarms"], nested: { entry: "assets/workbench.html" } }),
    ...
    const valid = await scanProductionBuild("C:/build", { readManifest: async () => JSON.stringify({ manifest_version: 3, incognito: "not_allowed", permissions: ["tabs", "tabGroups", "storage", "sessions", "alarms"] }), listFiles: async () => ["data.bin"], readFile: async () => new Uint8Array([0, 255, 1, 2]) });
    expect(valid.ok).toBe(true);
  });

  it("rejects mixed and non-Chrome targets", async () => {
    for (const target of [["chrome", "firefox"], ["firefox"]]) {
      const result = await scanProductionBuild("C:/build", { readManifest: async () => JSON.stringify({ manifest_version: 3, incognito: "not_allowed", permissions: ["tabs", "tabGroups", "storage", "sessions", "alarms"], targets: target }), listFiles: async () => [], readFile: async () => new Uint8Array() });
      expect(result.ok).toBe(false);
    }
  });
});
