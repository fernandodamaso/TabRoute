...
describe("workbench lease lifecycle", () => {
  it("reaps an old lease only when the process is dead", () => {
    ...
    expect(
      isLeaseReapable(lease, new Date("2026-01-01T00:03:00.001Z"), {
        kind: "dead"
      })
    ).toBe(true);
    expect(
  ...
  it("uses the conservative ten-minute rule when liveness is unavailable", () => {
    ...
    expect(
      isLeaseReapable(lease, new Date("2026-01-01T00:09:59.999Z"), unavailable)
    ).toBe(false);
    expect(
      isLeaseReapable(lease, new Date("2026-01-01T00:10:00.001Z"), unavailable)
    ).toBe(true);
  });
  ...
  it("accepts only an owned profile path and rejects unsafe cleanup targets", async () => {
    ...
    expect(
      manager.validateProfilePath(path.join(profileRoot, "run-1"), "run-1")
    ).toBe(true);
    expect(
      manager.validateProfilePath(
        path.join(profileRoot, "nested", "run-1"),
        "run-1"
      )
    ).toBe(false);
    for (const candidate of [
      path.join(root, "worktree", "child"),
      root,
      path.join(root, "sibling"),
      path.join(os.tmpdir(), "unrelated")
    ])
      expect(manager.validateProfilePath(candidate, "run-1")).toBe(false);
  ...
  it("performs initial cleanup plus exact retries and writes clean abandoned metadata", async () => {
    ...
    const manager = new LeaseManager({
      ...
      worktreePath: path.join(root, "worktree"),
      profileRoot,
      now: () => new Date("2020-01-01T00:03:00.000Z"),
      pid: 99,
      ...
      cleanup: async () => {
        attempts += 1;
        if (attempts < 4) throw new Error("locked");
      }
    });
  ...
  it("refreshes a five-second heartbeat until stopped", async () => {
    ...
    const calls: string[] = [];
    const handle = manager.startHeartbeat(
      "run-1",
      {
  ...
  it("writes parseable heartbeat JSON and permits release and reacquisition", async () => {
    const root = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-heartbeat-json-")
    );
    const manager = new LeaseManager({
      artifactRoot: path.join(root, "artifacts"),
      worktreePath: path.join(root, "worktree"),
    ...
    const refreshed = JSON.parse(
      await readFile(
        path.join(root, "artifacts", "run-1", "lease.json"),
        "utf8"
      )
    );
  ...
  it("fails closed when lease identity or timestamps have invalid runtime types", async () => {
    const root = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-bad-lease-types-")
    );
    const artifacts = path.join(root, "artifacts");
    await mkdir(path.join(artifacts, "bad"), { recursive: true });
    await writeFile(
      path.join(artifacts, "bad", "lease.json"),
      JSON.stringify({
        runId: "bad",
        pid: "42",
        startedAt: "not-date",
        heartbeat: "not-date",
        profilePath: path.join(root, "profiles", "bad"),
        status: "active"
      })
    );
    const manager = new LeaseManager({
  ...
  it("skips an invalid prior result instead of inventing abandoned metadata", async () => {
    const root = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-reap-invalid-result-")
    );
    const artifactRoot = path.join(root, "artifacts");
    const profileRoot = path.join(root, "profiles");
    const profile = path.join(profileRoot, "run-1");
  ...
  it("writes the exact bounded abandoned cleanup failure shape after all retries", async () => {
    const root = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-reap-failure-")
    );
    const artifactRoot = path.join(root, "artifacts");
    const profileRoot = path.join(root, "profiles");
    const profile = path.join(profileRoot, "run-1");
    ...
    const result = {
      ...
      cleanup: { profileRemoved: false },
      extensionId: "real-id",
      error: { message: "old error", details: { old: "field" } }
    };
    await writeFile(
      path.join(artifactRoot, "run-1", "lease.json"),
      JSON.stringify(lease)
    ...
    expect(results[0]).toEqual({
      ...
      lease: {
        ...lease,
        heartbeat: "2020-01-01T00:03:00.000Z",
        status: "abandoned"
      },
      extensionId: "real-id",
      cleanup: { profileRemoved: false, retainedPath: profile },
      error: { message: "x".repeat(8192) }
    });
    expect(validateRunResult(results[0])).toBe(true);
    expect(
      (results[0] as { error: { message: string } }).error.message.length
