...
it("maps one required row to a positive leaf and multiple rows to top-level AND", () => {
  const one = fromRule(
    rule({ kind: "host", operator: "exact", value: "example.com" })
  );
  ...
  const many = fromRule(
    rule({
      ...
      children: [
        ...
        { kind: "path", operator: "prefix", value: "/docs" }
      ]
    })
  );
  expect(many.kind).toBe("representable");
  if (many.kind === "representable")
    expect(toRule(many.draft).positive).toEqual({
      kind: "all",
      children: many.draft.required
    });
});
...
it("rejects nested OR or condition groups instead of silently flattening them", () => {
  ...
  if (result.kind === "unrepresentable")
    expect(result.reason).toMatch(/OR|nested|represent/i);
});
