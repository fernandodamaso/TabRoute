...
describe("workbench cross-process lock", () => {
  const runWorker = (args: string[]) =>
    new Promise<string>((resolve, reject) => {
      ...
      const child = spawn(process.execPath, [tsx, helper, ...args], {
      ...
      child.on("close", (code) =>
        code === 0
          ? resolve(output)
          : reject(new Error(`worker exited ${code}: ${errorOutput}`))
      );
    });
  it("serializes concurrent operations on one lock path", async () => {
    const lockPath = `${process.cwd()}/.vitest-workbench-lock-${process.pid}`;
    const first = createCrossProcessLock(lockPath, {
      retryDelayMs: 1,
      maxAttempts: 20
    });
    const second = createCrossProcessLock(lockPath, {
      retryDelayMs: 1,
      maxAttempts: 20
    });
  ...
  it("serializes two real Node contenders through the lock worker", async () => {
    ...
    const run = () =>
      new Promise<string>((resolve, reject) => {
        const child = spawn(process.execPath, [tsx, helper, lockPath], {
          stdio: ["ignore", "pipe", "pipe"]
        });
        let output = "";
        child.stdout.on("data", (chunk: Buffer) => {
    ...
    try {
      ...
      expect(outputs.every((output) => JSON.parse(output).ok === true)).toBe(
        true
      );
    } finally {
  ...
  it("allows exactly one of two real contenders to create lease eight", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-capacity-")
    );
    ...
    for (let i = 0; i < 7; i += 1) {
      ...
      await writeFile(
        path.join(run, "lease.json"),
        JSON.stringify({
          runId: `seed-${i}`,
          pid: process.pid,
          startedAt: new Date().toISOString(),
          heartbeat: new Date().toISOString(),
          profilePath: path.join(directory, "seed-profiles", `seed-${i}`),
          status: "active"
        })
      );
    }
    ...
    const run = (runId: string) =>
      new Promise<string>((resolve, reject) => {
        const child = spawn(
          process.execPath,
          [tsx, helper, artifactRoot, "lease", runId, directory],
          { stdio: ["ignore", "pipe", "pipe"] }
        );
        let output = "";
        child.stdout.on("data", (chunk: Buffer) => {
    ...
    try {
      const results = (
        await Promise.all([run("contender-a"), run("contender-b")])
      ).map((value) => JSON.parse(value));
      expect(results.filter((value) => value.ok === true)).toHaveLength(1);
      expect(
        results.filter((value) => value.code === "WORKBENCH_CAPACITY")
      ).toHaveLength(1);
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  ...
  it("does not remove a replacement lock after stale ownership revalidation", async () => {
    const lockPath = path.join(
      await mkdtemp(path.join(os.tmpdir(), "tabroute-stale-race-")),
      ".lock"
    );
    await (
      ...
      lockPath,
      JSON.stringify({ pid: 7, runId: "old", heartbeat: 0, token: "old-token" })
    );
  ...
  it("does not overwrite a replacement lock during heartbeat refresh", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-heartbeat-race-")
    );
    const lockPath = path.join(directory, ".lock");
    let heartbeat!: () => void;
    let replaced = false;
  ...
  it("keeps a live lock name present and parseable through heartbeat, release, and reacquire", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-heartbeat-live-")
    );
    const lockPath = path.join(directory, ".lock");
    let heartbeat!: () => void;
  ...
  it("refreshes heartbeat by metadata without rewriting the parseable owner record", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-heartbeat-mtime-")
    );
    const lockPath = path.join(directory, ".lock");
    let heartbeat!: () => void;
  ...
  it("recovers a malformed owner only after the conservative crash timeout", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-malformed-lock-")
    );
    const lockPath = path.join(directory, ".lock");
  ...
  it("does not remove a successor when an old owner releases during a replacement race", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-release-race-")
    );
    const lockPath = path.join(directory, ".lock");
    ...
    const third = createCrossProcessLock(lockPath, {
      isPidAlive: async () => true,
      retryDelayMs: 1,
      maxAttempts: 1
    });
    await expect(third.acquire()).rejects.toMatchObject({
      code: "WORKBENCH_CAPACITY"
    });
    await rm(directory, { recursive: true, force: true });
  });
  ...
  it("opens the post-validation stale window without deleting a newly acquired owner", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-stale-window-")
    );
    const lockPath = path.join(directory, ".lock");
    await (
      await import("node:fs/promises")
  ...
  it("serializes real artifact writes and prunes in one deterministic order", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-artifact-process-")
    );
    ...
    try {
      for (const runId of ["seed-a", "seed-b", "seed-c"]) {
        ...
        await writeFile(
          path.join(directory, runId, "status.json"),
          JSON.stringify({ status: "completed", terminalAt: Date.now() })
        );
        ...
        await writeFile(
          path.join(directory, runId, "trace", "seed.zip"),
          new Uint8Array(100)
        );
        await writeFile(
          path.join(directory, runId, ".artifact-index.json"),
          JSON.stringify([
            {
              relativePath: "trace/seed.zip",
              kind: "trace",
              capturedAt: Number(runId.at(-1))
            }
          ])
        );
      }
      ...
      await expect(
        access(path.join(directory, "seed-a", "trace", "seed.zip"))
      ).rejects.toBeDefined();
      await expect(
        readFile(path.join(directory, "seed-b", "lease.json"))
      ).resolves.toBeTruthy();
      await expect(
        readFile(path.join(directory, "seed-b", "status.json"))
      ).resolves.toBeTruthy();
      await expect(
        readFile(path.join(directory, "seed-b", "results.json"))
      ).resolves.toBeTruthy();
      await expect(
        readFile(path.join(directory, "seed-b", "error.json"))
      ).resolves.toBeTruthy();
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  ...
  it("serializes real concurrent orphan reaping with one clean result", async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-reap-process-")
    );
    const artifactRoot = path.join(directory, "artifacts");
    ...
    try {
      ...
      await writeFile(
        path.join(artifactRoot, "run-1", "lease.json"),
        JSON.stringify(lease)
      );
      await writeFile(
        path.join(artifactRoot, "run-1", "results.json"),
        JSON.stringify(result)
      );
      const outputs = await Promise.all([
        runWorker([artifactRoot, "reap", worktreePath, profileRoot]),
        runWorker([artifactRoot, "reap", worktreePath, profileRoot])
      ]);
      expect(
        outputs.filter((value) => JSON.parse(value).length === 1)
      ).toHaveLength(1);
      expect(
        outputs.filter((value) => JSON.parse(value).length === 0)
      ).toHaveLength(1);
      const persisted = JSON.parse(
        await readFile(path.join(artifactRoot, "run-1", "results.json"), "utf8")
      );
