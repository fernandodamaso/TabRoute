...
describe("workbench RunResult contracts", () => {
  it("accepts a worker timeout without an extension id", () => {
    expect(
      validateRunResult({
        ...
        phase: "worker",
        error: { message: "worker timed out" }
      })
    ).toBe(true);
  });
  ...
  it("requires a discovered extension id for manager and restart failures", () => {
    expect(
      validateRunResult({
        ...
        phase: "manager-query",
        error: { message: "manager timed out" }
      })
    ).toBe(false);
    expect(
      validateRunResult({
    ...
    expect(
      validateRunResult({
        ...
        code: "WORKBENCH_WORKER_TIMEOUT",
        phase: "restart-wake",
        error: { message: "restart wake failed" }
      })
    ).toBe(false);
    expect(
      validateRunResult({
  ...
  it("keeps unknown artifact failures bounded and without an invented id", () => {
    const result = createArtifactLimitFailure(
      {
        ...metadata,
        commandRecords: Array.from({ length: 5000 }, () => ({
          recordType: "event",
          mode: "fixture",
          source: "page",
          at: 1,
  ...
  it("exposes the required metadata caps as a stable public contract", () => {
    expect(REQUIRED_METADATA_CAPS).toEqual({
  ...
  it("rejects malformed started and abandoned metadata", () => {
    expect(validateStartedMetadata({ ...metadata, route: "not-a-route" })).toBe(
      false
    );
    expect(
      validateRunResult({
        ...
        ok: true,
        status: "abandoned",
        lease: { ...metadata.lease, status: "abandoned" },
        cleanup: { profileRemoved: true, retainedPath: "invented" }
      })
    ).toBe(false);
    expect(
      validateRunResult({
  ...
  it("rejects malformed nested records, readiness, screenshots, assertions, and cleanup", () => {
    ...
    expect(
      validateRunResult({
        ...
        code: "WORKBENCH_WORKER_TIMEOUT",
        phase: "worker",
        readiness: { workerDiscoveredAt: 42 },
        error: { message: "bad" }
      })
    ).toBe(false);
    expect(
  ...
  it("accepts exact pending request records and rejects pending extras", () => {
    ...
    expect(
      validateRunResult({
        ...metadata,
        commandRecords: [{ ...pending, endedAt: 2 }],
        ok: false,
        code: "WORKBENCH_WORKER_TIMEOUT",
        phase: "worker",
        error: { message: "pending" }
  ...
  it("validates save-rule records with the real rule field", () => {
    ...
    const record = {
      ...
      sequence: 1,
      scenarioId: "default",
      message: { kind: "manager-command", command: { kind: "saveRule", rule } },
      startedAt: 1,
      latencyMs: 0,
      state: "pending"
    };
        ...
        commandRecords: [
          {
            ...
            message: {
              ...
              command: { kind: "saveRule", input: rule }
            }
          }
        ],
        ok: false,
  ...
  it("accepts a RuleDraft without generated identity timestamps", () => {
    ...
    const record = {
      ...
      message: {
        kind: "manager-command",
        command: { kind: "saveRule", rule: draft }
      },
      startedAt: 1,
      latencyMs: 0,
      state: "pending"
  ...
  it("creates an artifact-limit value accepted by its own runtime contract", () => {
    const result = createArtifactLimitFailure(metadata, {
      message: "metadata too large"
    });
    ...
    ).toThrow("WORKBENCH_ARTIFACT_LIMIT");
  });
  ...
  it("requires exact command values and literal configuration values", () => {
    ...
    const command = (value: unknown) => ({
      ...metadata,
      commandRecords: [
        { ...base, message: { kind: "manager-command", command: value } }
      ],
      ok: false,
      code: "WORKBENCH_WORKER_TIMEOUT",
      phase: "worker",
    ...
    expect(
      validateRunResult(
        command({ kind: "setRuleEnabled", ruleId: "rule-1", enabled: "yes" })
      )
    ).toBe(false);
    expect(
      validateRunResult(
        command({ kind: "deleteRule", ruleId: "rule-1", extra: true })
      )
    ).toBe(false);
    const response = {
      ok: true,
    ...
    expect(
      validateRunResult({
        ...metadata,
        commandRecords: [{ ...base, state: "resolved", endedAt: 2, response }],
        ok: false,
        code: "WORKBENCH_WORKER_TIMEOUT",
        phase: "worker",
  ...
  it("keeps fixture scenario IDs and real worker generations disjoint", () => {
    ...
    expect(validateRunResult(result({ ...real, scenarioId: "default" }))).toBe(
      false
    );
    expect(validateRunResult(result(fixture))).toBe(true);
    expect(validateRunResult(result({ ...fixture, workerGeneration: 2 }))).toBe(
      false
    );
  });
  ...
  it("validates exact manager success and failure responses in resolved records", () => {
    ...
    const failure = {
      ok: false,
      error: { kind: "transport", message: "offline" }
    };
    expect(
      validateRunResult({
        ...metadata,
        commandRecords: [{ ...base, response: success }],
        ok: false,
        code: "WORKBENCH_WORKER_TIMEOUT",
        phase: "worker",
        error: { message: "x" }
    ...
    expect(
      validateRunResult({
        ...metadata,
        commandRecords: [{ ...base, response: { ...success, extra: true } }],
        ok: false,
        code: "WORKBENCH_WORKER_TIMEOUT",
        phase: "worker",
        error: { message: "x" }
    ...
    expect(
      validateRunResult({
        ...metadata,
        commandRecords: [{ ...base, response: failure }],
        ok: false,
        code: "WORKBENCH_WORKER_TIMEOUT",
        phase: "worker",
        error: { message: "x" }
    ...
    expect(
      validateRunResult({
        ...metadata,
        commandRecords: [{ ...base, response: { ...failure, extra: true } }],
        ok: false,
        code: "WORKBENCH_WORKER_TIMEOUT",
        phase: "worker",
        error: { message: "x" }
  ...
  it("distinguishes encoded reservation boundaries", () => {
    const jsonOverhead = new TextEncoder().encode(
      JSON.stringify({ value: "" })
    ).byteLength;
    const minus = new TextEncoder().encode(
      JSON.stringify({
        value: "x".repeat(
          REQUIRED_METADATA_RESERVATION_BYTES - jsonOverhead - 1
        )
      })
    ).byteLength;
    const exact = new TextEncoder().encode(
      JSON.stringify({
        value: "x".repeat(REQUIRED_METADATA_RESERVATION_BYTES - jsonOverhead)
      })
    ).byteLength;
    const plus = new TextEncoder().encode(
      JSON.stringify({
        value: "x".repeat(
          REQUIRED_METADATA_RESERVATION_BYTES - jsonOverhead + 1
        )
