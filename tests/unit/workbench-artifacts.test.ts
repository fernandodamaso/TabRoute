...
describe("workbench artifact retention", () => {
  ...
  it("does not classify required metadata by a category word in its parent path", async () => {
    ...
    const store = createArtifactStore({ root: run, runId: "run-video-1", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES + 20, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES + 100 });
  ...
  it("replaces an oversized result with a minimal failure that fits both reservations", async () => {
    ...
    const huge = { runId: "run-1", worktreePath: "worktree", buildPath: "build", profilePath: "profile", lease: { runId: "run-1", pid: 1, startedAt: "x", heartbeat: "x", profilePath: "profile", status: "active" as const }, cleanup: { profileRemoved: false }, commandRecords: Array.from({ length: 1000 }, () => "x".repeat(4096)) } satisfies ArtifactLimitSource & Record<string, unknown>;
  ...
  it("publishes a bounded failure when the full result fits alone but not with existing required metadata", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "tabroute-combined-overflow-"));
    const run = path.join(root, "run-1");
    const metadata: RunResultStartedMetadata & { lease: RunResultStartedMetadata["lease"] & { status: "active" } } = {
      status: "failed", runId: "run-1", worktreePath: "worktree", buildPath: "build", profilePath: "profile",
      mode: "fixture", url: "url", scenario: "default", route: "groups", deepLink: "none", commandRecords: Array.from({ length: 10 }, (_, index) => ({ recordType: "event" as const, mode: "fixture" as const, source: "page" as const, at: index, name: `event-${index}`, details: {} })),
      readiness: {}, screenshotPaths: [], assertions: [],
      lease: { runId: "run-1", pid: 1, startedAt: "2026-01-01T00:00:00.000Z", heartbeat: "2026-01-01T00:00:00.000Z", profilePath: "profile", status: "active" },
      cleanup: { profileRemoved: false }
    };
    const minimal = encodeUtf8(JSON.stringify(createArtifactLimitFailure(metadata, { message: "required metadata exceeds reserved artifact space" })));
    const full = encodeRequiredMetadata(metadata);
    expect(full.ok).toBe(true);
    if (!full.ok) return;
    expect(full.bytes.byteLength).toBeGreaterThan(minimal.byteLength);
    const store = createArtifactStore({ root: run, runId: "run-1", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES });
    await store.write("lease.json", new Uint8Array(REQUIRED_METADATA_RESERVATION_BYTES - minimal.byteLength), "lease");
    await store.writeRequiredResult(metadata);
    expect(JSON.parse(await readFile(path.join(run, "results.json"), "utf8"))).toMatchObject({ code: "WORKBENCH_ARTIFACT_LIMIT", phase: "artifact" });
    await rm(root, { recursive: true, force: true });
  });
  ...
  it("enforces store-level UTF-8 reservation boundaries and atomic overflow replacement", async () => {
    ...
    const make = (length: number) => ({ runId: "run-1", worktreePath: "worktree", buildPath: "build", profilePath: "profile", mode: "fixture", url: "url", scenario: "default", route: "groups", deepLink: "none", commandRecords: length ? Array.from({ length: 1000 }, () => "x".repeat(length)) : [], readiness: {}, screenshotPaths: [], assertions: [], lease: { runId: "run-1", pid: 1, startedAt: "x", heartbeat: "x", profilePath: "profile", status: "active" as const }, cleanup: { profileRemoved: false }, error: { message: "x" } });
  ...
  it("accepts required metadata at exact encoded size and rejects one byte below", async () => {
    const metadata = { runId: "run-1", worktreePath: "worktree", buildPath: "build", profilePath: "profile", lease: { runId: "run-1", pid: 1, startedAt: "x", heartbeat: "x", profilePath: "profile", status: "active" as const }, cleanup: { profileRemoved: false }, commandRecords: Array.from({ length: 1000 }, () => ({ name: "x".repeat(2000) })) } satisfies ArtifactLimitSource & Record<string, unknown>;
    ...
    const below = createArtifactStore({ root: path.join(root, "below"), runId: "below", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES - 1, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES - 1 });
    await expect(below.writeRequiredResult(metadata)).rejects.toThrow("WORKBENCH_ARTIFACT_LIMIT");
    const exact = createArtifactStore({ root: path.join(root, "exact"), runId: "exact", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES });
    await exact.writeRequiredResult(metadata);
    const plusRoot = await mkdtemp(path.join(os.tmpdir(), "tabroute-reservation-plus-"));
    const plus = createArtifactStore({ root: path.join(plusRoot, "plus"), runId: "plus", globalRoot: plusRoot, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES + 1, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES + 1 });
  ...
  it("enforces affected-run required bytes at minus, exact, and plus boundaries", async () => {
    ...
    try {
      ...
      await expect(minus.write("lease.json", new Uint8Array(REQUIRED_METADATA_RESERVATION_BYTES - 1), "lease")).rejects.toThrow("WORKBENCH_ARTIFACT_LIMIT");
  ...
  it("keeps separate required reservations for multiple runs in the global budget", async () => {
    ...
    try {
      const first = createArtifactStore({ root: path.join(root, "run-a"), runId: "run-a", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES + 100, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES * 2 });
      const second = createArtifactStore({ root: path.join(root, "run-b"), runId: "run-b", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES + 100, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES * 2 });
  ...
  it("leaves only the remaining required headroom for optional writes", async () => {
    ...
    const store = createArtifactStore({ root: run, runId: "run-1", globalRoot: root, activeBudgetBytes: reservation + 200, globalBudgetBytes: reservation + 200 });
  ...
  it("does not treat nested required-looking names as root required metadata", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "tabroute-nested-required-"));
    const store = createArtifactStore({ root: path.join(root, "run-1"), runId: "run-1", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES });
    await expect(store.write("nested/results.json", new Uint8Array(1), "result", { capturedAt: 1 })).rejects.toThrow("WORKBENCH_ARTIFACT_LIMIT");
    await rm(root, { recursive: true, force: true });
  });

  it("preserves exact required headroom at affected optional-write boundaries", async () => {
    const payloadBytes = 7;
    const indexBytes = encodeUtf8(JSON.stringify([{ relativePath: "trace/a.zip", kind: "trace", capturedAt: 1 }])).byteLength;
    for (const delta of [-1, 0, 1]) {
      const root = await mkdtemp(path.join(os.tmpdir(), `tabroute-affected-optional-${delta}-`));
      const run = path.join(root, "run-1");
      const store = createArtifactStore({ root: run, runId: "run-1", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES + payloadBytes + indexBytes + delta, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES * 2 });
      const write = store.write("trace/a.zip", new Uint8Array(payloadBytes), "trace", { capturedAt: 1 });
      if (delta < 0) await expect(write).rejects.toThrow("WORKBENCH_ARTIFACT_LIMIT");
  ...
  it("preserves exact required headroom at global optional-write boundaries", async () => {
    ...
    const indexBytes = encodeUtf8(JSON.stringify([{ relativePath: "trace/a.zip", kind: "trace", capturedAt: 1 }])).byteLength;
    for (const delta of [-1, 0, 1]) {
      const root = await mkdtemp(path.join(os.tmpdir(), `tabroute-global-optional-${delta}-`));
      const run = path.join(root, "run-1");
      const store = createArtifactStore({ root: run, runId: "run-1", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES * 2, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES + payloadBytes + indexBytes + delta });
      const write = store.write("trace/a.zip", new Uint8Array(payloadBytes), "trace", { capturedAt: 1 });
      if (delta < 0) await expect(write).rejects.toThrow("WORKBENCH_ARTIFACT_LIMIT");
      else await expect(write).resolves.toBeUndefined();
      await rm(root, { recursive: true, force: true });
  ...
  it("preserves global headroom for every active run when logs cannot be pruned", async () => {
    const payloadBytes = 7;
    const indexBytes = encodeUtf8(JSON.stringify([{ relativePath: "worker.log", kind: "log", capturedAt: 1 }])).byteLength;
    for (const delta of [-1, 0]) {
      const root = await mkdtemp(path.join(os.tmpdir(), `tabroute-global-active-${delta}-`));
      const budget = REQUIRED_METADATA_RESERVATION_BYTES * 2 + payloadBytes + indexBytes + delta;
      const first = createArtifactStore({ root: path.join(root, "run-a"), runId: "run-a", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES * 2, globalBudgetBytes: budget });
      const second = createArtifactStore({ root: path.join(root, "run-b"), runId: "run-b", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES * 2, globalBudgetBytes: budget });
      await first.write("worker.log", new Uint8Array(payloadBytes), "log", { capturedAt: 1 });
      const write = second.write("lease.json", new Uint8Array(REQUIRED_METADATA_RESERVATION_BYTES), "lease");
      if (delta < 0) await expect(write).rejects.toThrow("WORKBENCH_ARTIFACT_LIMIT");
      else await expect(write).resolves.toBeUndefined();
      await rm(root, { recursive: true, force: true });
    }
  ...
  it("uses each run's index when global pressure prunes optional evidence", async () => {
    ...
    const first = createArtifactStore({ root: path.join(root, "run-a"), runId: "run-a", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES + 1000, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES * 2 + 400 });
    const second = createArtifactStore({ root: path.join(root, "run-b"), runId: "run-b", globalRoot: root, activeBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES + 1000, globalBudgetBytes: REQUIRED_METADATA_RESERVATION_BYTES * 2 + 400 });
