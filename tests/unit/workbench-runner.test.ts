...
describe("workbench build isolation", () => {
  it("creates graph-specific output under the current worktree and never .output", () => {
    ...
    expect(
      resolveBuildOutput(worktree, "run-b", "production").buildPath
    ).not.toContain(".output");
  });

  it("refuses to build a checkout other than process.cwd()", async () => {
    const other = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-other-worktree-")
    );
...
describe("Chromium extension lifecycle contracts", () => {
  it("derives the extension ID from a validated service-worker URL", () => {
    ...
    expect(
      parseExtensionWorkerUrl(`chrome-extension://${id}/background.js`)
    ).toBe(id);
    expect(() =>
      parseExtensionWorkerUrl(`https://${id}/background.js`)
    ).toThrow("WORKBENCH_ARGUMENT");
    expect(() =>
      parseExtensionWorkerUrl("chrome-extension://fixed/background.js")
    ).toThrow("WORKBENCH_ARGUMENT");
  });

  it("launches bundled Chromium headlessly with only the run build loaded", () => {
    const buildPath = path.resolve(
      "C:/repo/.workbench/tmp/run-a/workbench/chrome-mv3"
    );
  ...
  it("retries only the receiving-end startup race at 250 ms cadence", async () => {
    ...
    expect(
      isReceivingEndStartupRace(new Error("RECEIVING END DOES NOT EXIST"))
    ).toBe(true);
    expect(isReceivingEndStartupRace(new Error("connection closed"))).toBe(
      false
    );
  });
  ...
  it("does not retry any other manager error", async () => {
    ...
    await expect(settleManagerQuery({ request })).rejects.toThrow(
      "connection closed"
    );
...
describe("runner cleanup evidence", () => {
  it("always closes the session and removes the owned profile", async () => {
    const root = await mkdtemp(
      path.join(os.tmpdir(), "tabroute-runner-cleanup-")
    );
    temporary.push(root);
  ...
  it("retains and reports the profile when cleanup exhausts its retries", async () => {
    ...
    await expect(
      ...
      retainedPath: path.join(os.tmpdir(), "tabroute-workbench", "run-a"),
      error: "profile busy"
    });
  ...
  it("prints the retained result path as one stable line", () => {
    expect(
      formatRetainedResultPath(
        path.resolve("C:/repo/.workbench/artifacts/run-a/results.json")
      )
    ).toBe(
      `Workbench result: ${path.resolve("C:/repo/.workbench/artifacts/run-a/results.json")}`
    );
  });
});
