...
it("exposes the workbench controls outside an exact 520 by 600 preview", async () => {
  ...
  expect(
    screen.getByTestId("workbench-preview").getAttribute("data-preview-width")
  ).toBe("520");
  expect(
    screen.getByTestId("workbench-preview").getAttribute("data-preview-height")
  ).toBe("600");
  expect(
    (screen.getByTestId("workbench-preview") as HTMLElement).style.width
  ).toBe("520px");
  expect(
    (screen.getByTestId("workbench-preview") as HTMLElement).style.height
  ).toBe("600px");
  for (const control of [
    ...
    expect(
      document.querySelector(`[data-workbench-control="${control}"]`)
    ).toBeTruthy();
  }
...
it("keeps the loading marker pending until the fixture response is released", async () => {
  setSearch(
    "?workbench=1&mode=fixture&route=groups&scenario=wb%3Aloading&deep-link=none&latency=0&failure=none"
  );
  const user = userEvent.setup();
  render(<WorkbenchOptionsApp />);
  ...
  expect(
    document.querySelector('[data-workbench-status="manager-pending"]')
  ).toBeTruthy();
  const release = screen.getByRole("button", {
    name: "Release pending response"
  });
  ...
  await waitFor(() =>
    expect(
      document.querySelector('[data-workbench-status="manager-ready"]')
    ).toBeTruthy()
  );
...
it("opens the selected deep-link editor and confirmation dialog", async () => {
  setSearch(
    "?workbench=1&mode=fixture&route=rules&scenario=wb%3Aedit-rule&deep-link=edit-rule%3A00000000-0000-4000-8000-000000000101&latency=0&failure=none"
  );
  ...
  setSearch(
    "?workbench=1&mode=fixture&route=rules&scenario=wb%3Aconfirmation-overlay&deep-link=confirm-delete%3A00000000-0000-4000-8000-000000000101&latency=0&failure=none"
  );
  render(<WorkbenchOptionsApp />);
  expect(
...
it("does not expose workbench markers or controls on popup and normal options surfaces", () => {
  ...
  expect(
    document.querySelector(
      '[data-workbench-marker="TABROUTE_DEV_WORKBENCH_V1"]'
    )
  ).toBeNull();
...
it("reports the latest terminal typed failure instead of an earlier pending query", () => {
  ...
  render(
    <WorkbenchHost
      ...
      real={{
        request: async () => ({
          ok: false,
          error: { kind: "transport", message: "No response" }
        })
      }}
      records={records}
  ...
  expect(
    document.querySelector('[data-workbench-status="manager-error"]')
  ).toBeTruthy();
  expect(
    screen.getByRole("status", { name: "Result status" }).textContent
  ).toContain("error");
});
...
it("lets a deep-link UUID be typed before committing the validated URL", async () => {
  ...
  expect((uuid as HTMLInputElement).value).toBe(
