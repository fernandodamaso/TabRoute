...
it("keeps the navigator fixed while the inspector owns scrolling", () => {
  ...
  expect(
    document
      .querySelector(".groups-navigator")
      ?.classList.contains("groups-scroll-owner")
  ).toBe(false);
  expect(
    document
      .querySelector(".groups-inspector")
      ?.classList.contains("groups-scroll-owner")
  ).toBe(true);
});
...
it("exposes group identity, color, emoji, enablement, and fallback protection", async () => {
  ...
  expect(
    (screen.getByRole("checkbox", { name: "Group On" }) as HTMLInputElement)
      .disabled
  ).toBe(true);
...
it("supports group creation and deletion through typed commands", async () => {
  ...
  expect(command).toHaveBeenCalledWith(
    expect.objectContaining({
      command: expect.objectContaining({ kind: "createGroup" })
    })
  );
  await user.click(screen.getByRole("button", { name: /Delete group/ }));
  expect(command).toHaveBeenCalledWith(
    expect.objectContaining({
      command: expect.objectContaining({ kind: "deleteGroup" })
    })
  );
});
