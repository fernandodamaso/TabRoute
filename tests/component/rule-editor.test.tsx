...
it("renders IF, AND, NOT, and one THEN outcome without nested groups", async () => {
  ...
  expect(
    screen.queryByRole("button", { name: /Add nested|Add OR/ })
  ).toBeNull();
});
...
it("blocks invalid regular expressions and focuses the first error", async () => {
  ...
  await user.selectOptions(
    screen.getByRole("combobox", { name: "Condition field 1" }),
    "url"
  );
  await user.selectOptions(
    screen.getByRole("combobox", { name: "Condition operator 1" }),
    "regex"
  );
...
it("edits bounded THEN actions and preserves the duplicate pattern", async () => {
  ...
  await user.selectOptions(
    screen.getByRole("combobox", { name: "Duplicate policy action" }),
    "pattern"
  );
  await user.type(
    screen.getByRole("textbox", { name: "Duplicate pattern" }),
    "docs/*"
  );
        ...
        rule: expect.objectContaining({
          actions: [
            ...
            { kind: "makePersistent" },
            {
              ...
              policy: { kind: "pattern", pattern: "docs/*" }
            },
