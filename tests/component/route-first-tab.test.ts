...
import type { ChromeInventory, ChromeMutationPort, ChromeTabSnapshot } from "../../src/chrome/types";
...
function fakePort(initial: ChromeInventory) {
  ...
  const port: ChromeMutationPort = {
    ...
    async groupTabs(input) {
      ...
      inventory.groups = [...inventory.groups, { id, windowId: input.windowId, title: "", color: "grey" as const, collapsed: false, shared: false }].filter(
        (group, index, groups) => groups.findIndex((candidate) => candidate.id === group.id) === index
      );
      inventory.tabs = inventory.tabs.map((candidate) => input.tabIds.includes(candidate.id) ? { ...candidate, chromeGroupId: id } : candidate);
      return id;
    },
    async moveTabs() { calls.push("move-tabs"); },
    async updateGroup(groupId, patch) {
      calls.push("update-group");
      inventory.groups = inventory.groups.map((group) => group.id === groupId ? { ...group, ...patch } : group);
    }
  };
...
it("holds a newly created tab until it has a committed supported URL", async () => {
  ...
  const fake = fakePort({ windows: [{ id: 1, focused: true, incognito: false, type: "normal" }], tabs: [tab({ url: undefined, status: "loading" })], groups: [], capturedAt: 1 });
  const controller = createTabRouteController({ configuration, chrome: fake.port, session: createMemorySessionRepository() });
...
it("routes an unmatched routable tab through the Action Engine into lazy Other", async () => {
  const configuration = createDefaultConfiguration(() => "00000000-0000-4000-8000-000000000001");
  const fake = fakePort({ windows: [{ id: 1, focused: true, incognito: false, type: "normal" }], tabs: [tab()], groups: [], capturedAt: 1 });
  const controller = createTabRouteController({ configuration, chrome: fake.port, session: createMemorySessionRepository() });
...
it("reuses each normal window's fallback group after fresh association reconstruction", async () => {
  ...
  expect(fake.calls).toEqual(["reuse-group", "update-group", "reuse-group", "update-group"]);
  expect(fake.getInventory().tabs.find((candidate) => candidate.id === first.id)?.chromeGroupId).toBe(13);
  expect(fake.getInventory().tabs.find((candidate) => candidate.id === second.id)?.chromeGroupId).toBe(14);
});
