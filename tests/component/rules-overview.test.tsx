  ...
  const configuration: Configuration = {
    ...
    rules: [
      ...
      },
      {
        ...
        targetGroupId: fallbackId,
        priority: 10,
        positive: { kind: "title", operator: "contains", value: "Paused" },
        negative: [],
        actions: [{ kind: "group" }],
        enabled: true,
        pausedUntil: "restart",
      ...
      },
      {
        ...
        targetGroupId: fallbackId,
        priority: 1,
        positive: { kind: "pinned", value: true },
        negative: [],
        actions: [{ kind: "group" }],
        enabled: false,
        createdAt: 1,
...
it("shows rule details and routes enabled/paused changes through one command", async () => {
  ...
  expect(command).toHaveBeenCalledWith(
    expect.objectContaining({
      command: expect.objectContaining({ kind: "setRuleEnabled" })
    })
  );
  await user.click(screen.getAllByRole("button", { name: /Pause rule/ })[0]!);
  expect(command).toHaveBeenCalledWith(
    expect.objectContaining({
      command: expect.objectContaining({ kind: "setRulePaused" })
    })
  );
});
...
it("duplicates through the background command and exposes Edit", async () => {
  ...
  expect(command).toHaveBeenCalledWith(
    expect.objectContaining({
      command: expect.objectContaining({ kind: "duplicateRule" })
    })
  );
