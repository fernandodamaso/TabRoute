...
import { launchExtensionSession, type ExtensionSession } from "../../scripts/workbench/browser";
...
async function createFixtureSession(): Promise<FixtureSession> {
  ...
  const build = await buildExtension({ worktreePath: process.cwd(), runId: buildRunId, graph: "workbench" });
...
async function openScenario(page: Page, extensionId: string, scenarioId: string): Promise<void> {
  await page.goto(scenarioUrl(extensionId, scenarioId));
  const scenario = getScenarioDefinition(scenarioId);
  if (scenario.id === "wb:loading") {
    await expect(page.locator('[data-workbench-status="manager-pending"]')).toBeVisible({ timeout: MANAGER_READY_TIMEOUT });
    await page.getByRole("button", { name: "Release pending response" }).click();
    await page.waitForSelector('[data-workbench-status="manager-ready"]', { timeout: MANAGER_READY_TIMEOUT });
    return;
  }
  ...
  const statusSelector = scenario.expected.status === "error"
    ? '[data-workbench-status="manager-error"]'
    : '[data-workbench-status="manager-ready"]';
  await page.waitForSelector(statusSelector, { timeout: MANAGER_READY_TIMEOUT });
}

async function assertScenarioPublicState(page: Page, scenarioId: string): Promise<void> {
  const scenario = getScenarioDefinition(scenarioId);
  const preview = page.locator(".workbench-preview");
  if (scenario.expected.heading !== "Groups" || scenario.id !== "wb:empty-groups") {
    await expect(preview.getByRole("heading", { name: scenario.expected.heading })).toBeVisible();
  }
  for (const snippet of scenario.expected.snippets ?? []) {
    if (scenario.id === "wb:mixed-rules-overview") {
      await expect(preview.locator(".rule-status-active").first()).toBeVisible();
      continue;
    }
    if (scenario.id === "wb:empty-groups") {
      await expect(preview.getByRole("heading", { name: "Other" })).toBeVisible();
      continue;
    }
    if (scenario.id === "wb:populated-persistent-tabs") {
      await preview.locator(".groups-list button").filter({ hasText: "Work" }).first().click();
      await expect(preview.getByRole("listitem").filter({ hasText: "docs.example.test" })).toBeVisible();
      continue;
    }
    await expect(preview.getByText(snippet, { exact: false })).toBeVisible();
  }
  if (scenario.expected.dialogTitle) {
    await expect(preview.getByRole("dialog", { name: scenario.expected.dialogTitle })).toBeVisible();
  }
  if (scenario.id === "wb:validation-error") {
    await preview.getByRole("button", { name: "Save rule" }).click();
    await expect(preview.getByRole("alert")).toContainText("Fixture validation failure");
    return;
  }
  const status = await page.locator("[data-workbench-status]").getAttribute("data-workbench-status");
  if (scenario.expected.status === "error") expect(status).toBe("manager-error");
  else expect(status).toBe("manager-ready");
}

async function previewDimensions(page: Page): Promise<{ width: number; height: number }> {
  const preview = page.locator(".workbench-preview");
...
async function readCommandRecords(page: Page): Promise<ManagerTransportRecord[]> {
...
test("default scenario", async () => {
  ...
  if (result.ok && result.status === "completed") {
    ...
    const logPath = path.join(process.cwd(), ".workbench", "artifacts", result.runId, "runner.log");
...
test("route navigation and route focus", async () => {
  ...
  try {
    ...
    for (const route of routes) {
      ...
      await expect(page.locator(".workbench-preview").getByRole("heading", { name: route.heading })).toBeVisible();
      await expect.poll(async () => page.locator(`main[data-route-focus="${route.focus}"]`).evaluate((element) => element === document.activeElement)).toBe(true);
    }
  } finally {
    await page.close();
    await disposeFixtureSession(fixture);
...
test("deep links open the expected editor and confirmation overlay", async () => {
  ...
  try {
    ...
    await page.goto(scenarioUrl(fixture.session.extensionId, "wb:confirmation-overlay"));
    await page.waitForSelector('[data-workbench-status="manager-ready"]', { timeout: MANAGER_READY_TIMEOUT });
...
test("header and primary navigation stay fixed while feature bodies scroll", async () => {
  ...
  try {
    ...
    const headerOverflow = await page.locator(".manager-header").evaluate((element) => window.getComputedStyle(element).overflowY);
    const navOverflow = await page.locator(".manager-primary-nav").evaluate((element) => window.getComputedStyle(element).overflowY);
    ...
    const groupsListScrollable = await page.locator(".groups-list").evaluate((element) => element.scrollHeight > element.clientHeight);
    const inspectorScrollable = await page.locator(".groups-inspector").evaluate((element) => element.scrollHeight > element.clientHeight);
    ...
    await page.waitForSelector('[data-workbench-status="manager-ready"]', { timeout: MANAGER_READY_TIMEOUT });
    const rulesScrollable = await page.locator(".workbench-preview .manager-page-scroll").evaluate((element) => element.scrollHeight > element.clientHeight);
...
test("confirmation overlay traps focus and Escape restores it", async () => {
  ...
  const page = await fixture.session.context.newPage();
  try {
    await openScenario(page, fixture.session.extensionId, "wb:confirmation-overlay");
    const preview = page.locator(".workbench-preview");
    ...
    await expect.poll(async () => preview.locator(".confirmation-dialog").evaluate((element) => element.contains(document.activeElement))).toBe(true);
...
test("latency failure reset and command records stay ordered", async () => {
  ...
  try {
    ...
    const queryRecord = records.find((record) => record.recordType === "request" && record.message.kind === "manager-query");
    ...
    await page.goto(`chrome-extension://${fixture.session.extensionId}/options.html${serializeWorkbenchUrl(failingState)}`);
    await page.waitForSelector('[data-workbench-status="manager-error"]', { timeout: MANAGER_READY_TIMEOUT });
    records = await readCommandRecords(page);
    const rejected = records.filter((record) => record.recordType === "request" && record.state === "rejected");
    expect(rejected.length).toBeGreaterThan(0);
    const sequences = records
      .filter((record): record is Extract<ManagerTransportRecord, { recordType: "request" }> => record.recordType === "request")
    ...
    await page.goto(`chrome-extension://${fixture.session.extensionId}/options.html${serializeWorkbenchUrl(recoveredState)}`);
    await page.waitForSelector('[data-workbench-status="manager-ready"]', { timeout: MANAGER_READY_TIMEOUT });
    records = await readCommandRecords(page);
    const latestQuery = [...records]
      .reverse()
      .find((record) => record.recordType === "request" && record.message.kind === "manager-query" && record.state === "resolved");
...
test("reset restores the fixture session after controls change", async () => {
  ...
  try {
    ...
    await expect(page.locator('[data-workbench-status="manager-ready"]')).toBeVisible({ timeout: MANAGER_READY_TIMEOUT });
    const records = await readCommandRecords(page);
    expect(records.some((record) => record.recordType === "request" && record.message.kind === "manager-query")).toBe(true);
  } finally {
    await page.close();
    await disposeFixtureSession(fixture);
  }
...
test("loading scenario stays pending until release", async () => {
  ...
  try {
    ...
    await expect(page.locator('[data-workbench-status="manager-pending"]')).toBeVisible();
    await expect(page.getByText("Loading", { exact: true })).toBeVisible();
    await page.getByRole("button", { name: "Release pending response" }).click();
    await page.waitForSelector('[data-workbench-status="manager-ready"]', { timeout: MANAGER_READY_TIMEOUT });
    await expect(page.locator(".workbench-preview").getByRole("heading", { name: "Groups" })).toBeVisible();
  } finally {
    await page.close();
    await disposeFixtureSession(fixture);
  }
