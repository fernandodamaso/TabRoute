...
function validateState(state: WorkbenchUrlState): WorkbenchUrlState {
  ...
  if (
    ...
    !UUID_PATTERN.test(state.deepLink.ruleId)
  )
    return fail("deep-link rule id must be a UUID");
  if (
    !Number.isInteger(state.latencyMs) ||
    state.latencyMs < 0 ||
    state.latencyMs > 5000
  )
    return fail("latency must be an integer from 0 through 5000");
  if (state.failure.mode !== "none") {
    if (
      !(["query", "command", "validation", "offline"] as const).includes(
        state.failure.mode
      )
    )
      return fail("invalid failure mode");
  ...
  if (state.mode === "real") {
    if (state.scenarioId !== "wb:default")
      return fail("real mode accepts only wb:default");
    if (state.latencyMs !== 0) return fail("real mode latency must be zero");
    if (state.failure.mode !== "none")
      return fail("real mode failure must be none");
  }
  return state;
}
...
export function parseWorkbenchSearch(search: string): WorkbenchUrlState {
  ...
  if (!SCENARIO_IDS.includes(scenarioId as (typeof SCENARIO_IDS)[number]))
