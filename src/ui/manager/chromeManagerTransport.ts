...
function isView(value: unknown): value is ManagerViewMetadata {
  ...
  return value.width === 520 &&
    ...
    value.routes.every((route) =>
      route === "groups" || route === "rules" || route === "activity" || route === "settings"
    );
}
...
function isViewFixture(value: unknown): value is ManagerViewFixture {
  ...
  return Object.values(value.persistentTabsByGroup).every((fixture) => {
    ...
    const validState = fixture.state === "loading" || fixture.state === "empty" ||
      fixture.state === "populated" || fixture.state === "disabled" || fixture.state === "error";
...
function isFailure(value: unknown): value is ManagerFailure {
  ...
  const validKind = kind === "validation" || kind === "reference" ||
    kind === "persistence" || kind === "offline" || kind === "transport";
  return validKind &&
...
function normalizeResponse(value: unknown): ManagerResponse | undefined {
  ...
  if (!isRecord(value) || value.ok !== true || !isView(value.view)) return undefined;
  if (value.viewFixture !== undefined && !isViewFixture(value.viewFixture)) return undefined;
  try {
    ...
    return {
      ...
      configuration,
      view: value.view,
      ...(value.viewFixture === undefined ? {} : { viewFixture: value.viewFixture })
    };
  } catch {
    return undefined;
  }
...
function failure(kind: ManagerFailure["error"]["kind"], code: string, message: string): ManagerFailure {
  return { ok: false, error: { kind, code, message } };
}

function classifyError(error: unknown): ManagerFailure {
  const message = error instanceof Error ? error.message : "Manager transport failed";
  const lower = message.toLowerCase();
  if ((typeof navigator !== "undefined" && navigator.onLine === false) || lower.includes("offline"))
  ...
  return new Promise((resolve, reject) => {
    try {
      chrome.runtime.sendMessage(message, (response) => {
        ...
        if (lastError) {
          reject(new ChromeManagerTransportError(
            "RUNTIME_LAST_ERROR",
...
function withTimeout<T>(promise: Promise<T>, timeoutMs: number): Promise<T> {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new ChromeManagerTransportError("TIMEOUT", "Manager request timed out")),
      timeoutMs
    );
    promise.then(
...
export function createChromeManagerTransport(input: {
  ...
  function emit(record: ManagerTransportRecord) {
    try { input.onRecord?.(record); } catch { /* recorders are observational */ }
  }
  ...
  return {
    async request(message) {
      ...
      try {
        ...
        if (raw === undefined) {
          const result = failure("transport", "NO_RESPONSE", "Manager runtime returned no response");
          emit({
            recordType: "request",
            state: "rejected",
            mode: "real",
        ...
        if (!response) {
          const result = failure("transport", "INVALID_RESPONSE", "Manager runtime returned an invalid response");
