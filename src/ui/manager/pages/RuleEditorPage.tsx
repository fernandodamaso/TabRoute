...
function setPlacement(
  ...
  return [
    { kind: nextPlacement } as RuleAction,
    ...bounded.filter(
...
function duplicatePolicyAction(actions: readonly RuleAction[]) {
  return actions.find(
    (
      action
    ): action is { kind: "setDuplicatePolicy"; policy: DuplicatePolicy } =>
...
function collapseAction(actions: readonly RuleAction[]) {
  return actions.find(
    (action): action is { kind: "setCollapsed"; collapsed: boolean } =>
...
export function RuleEditorPage({
  ...
  const loaded = rule
    ? fromRule(rule)
    : {
        kind: "representable" as const,
        draft: newDraft(configuration, prefill)
      };
  const [draft, setDraft] = useState<FlatRuleDraft>(
    loaded.kind === "representable" ? loaded.draft : newDraft(configuration)
  );
  const [error, setError] = useState<string>();
  const firstValueRef = useRef<HTMLInputElement | HTMLSelectElement>(null);
  if (loaded.kind === "unrepresentable")
    return (
      <div className="rule-editor">
        <h1>Rule editor</h1>
        <p role="alert">{loaded.reason}</p>
        <p>Cancel keeps the stored legacy rule unchanged.</p>
  ...
  const save = async () => {
    try {
      ...
      const withRule = draft.id
        ? configuration.rules.map((item) =>
            item.id === draft.id ? { ...item, ...candidate } : item
          )
        : [
            ...configuration.rules,
            {
      ...
      const response = await command({
        ...
        command: { kind: "saveRule", rule: candidate }
      });
  ...
  return (
    <div className="rule-editor">
      <div className="rule-editor-heading">
        <div>
          <p className="manager-eyebrow">RULES</p>
          <h1>{rule ? "Edit rule" : "New rule"}</h1>
        </div>
        <span>IF / AND / NOT</span>
      </div>
      <section className="manager-card">
        <h2>IF</h2>
        <div className="condition-rows">
          {draft.required.map((condition, index) => (
            <ConditionRow
              key={index}
              condition={condition}
              index={index + 1}
              ...
              onChange={(next) =>
                setDraft((current) => ({
                  ...current,
                  required: current.required.map((item, itemIndex) =>
        ...
        <button
          ...
          onClick={() =>
            setDraft((current) => ({
              ...current,
              required: [...current.required, defaultLeaf()]
            }))
          }
        >
          Add condition
      ...
      <section className="manager-card">
        <h2>AND</h2>
        <p className="manager-note">All required conditions must match.</p>
      </section>
      <section className="manager-card">
        <h2>NOT</h2>
        <div className="condition-rows">
          {draft.exceptions.map((condition, index) => (
            <ConditionRow
              key={index}
              condition={condition}
              index={index + 1}
        ...
        <button
          ...
          onClick={() =>
            setDraft((current) => ({
              ...
              exceptions: [
                ...current.exceptions,
                { kind: "title", operator: "contains", value: "Blocked" }
              ]
            }))
          }
        >
      ...
      <section className="manager-card">
        ...
        <label>
          ...
          <select
            ...
            disabled={currentPlacement === "ungroup"}
            onChange={(event) =>
              setDraft((current) => ({
                ...current,
            ...
            onChange={(event) =>
              setDraft((current) => ({
                ...
                actions: replaceAction(
                  current.actions,
                  "makePersistent",
                  event.target.checked ? { kind: "makePersistent" } : undefined
                )
              }))
            }
          />{" "}
        ...
        <label>
          ...
          <select
            ...
            value={currentDuplicatePolicy?.kind ?? "none"}
            onChange={(event) =>
              setDraft((current) => ({
                ...current,
                actions: replaceAction(
                    ...
                    : {
                        ...
                        policy:
                          event.target.value === "pattern"
                            ...
                            : {
                                kind: event.target.value as Exclude<
                                  DuplicatePolicy["kind"],
                                  "pattern"
                                >
                              }
                      }
        ...
        {currentDuplicatePolicy?.kind === "pattern" && (
          <label>
            Duplicate pattern
            <input
              aria-label="Duplicate pattern"
              value={currentDuplicatePolicy.pattern}
              onChange={(event) =>
                setDraft((current) => ({
                  ...current,
                  actions: replaceAction(
                    ...
                    {
                      kind: "setDuplicatePolicy",
                      policy: { kind: "pattern", pattern: event.target.value }
                    }
                  )
                }))
              }
              ...
              setDraft((current) => ({
                ...
                actions: replaceAction(
                  ...
                  event.target.value === "none"
                    ...
                    : {
                        kind: "setCollapsed",
                        collapsed: event.target.value === "collapsed"
                      }
                )
              }))
            }
