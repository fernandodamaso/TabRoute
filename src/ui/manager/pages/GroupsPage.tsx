...
export function GroupsPage({
  ...
  const selected =
    configuration.groups.find((group) => group.id === selectedId) ??
      ...
      <GroupNavigator
        ...
        onAdd={() =>
          void command({
            ...
            command: {
              kind: "createGroup",
              input: { name: "New group", color: "blue" }
            }
          })
        }
        onDelete={(id) =>
          void command({
            kind: "manager-command",
            command: { kind: "deleteGroup", groupId: id }
          })
        }
      />
      <GroupInspector
        ...
        configuration={configuration}
        command={command}
        viewFixture={viewFixture?.persistentTabsByGroup[selected.id]}
