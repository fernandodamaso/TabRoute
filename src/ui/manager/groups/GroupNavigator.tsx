...
export function GroupNavigator({
  ...
  return (
    <aside className="groups-navigator" aria-label="Groups navigator">
      <div className="groups-navigator-heading">
      ...
      <div className="groups-list">
        {ordered.map((group) => (
          ...
          >
            ...
            {group.isFallback && <span className="group-role">Fallback</span>}
          </button>
        ))}
      </div>
      {selected && !selected.isFallback && (
        <button
          type="button"
          className="group-delete"
          onClick={() => onDelete(selected.id)}
        >
          Delete group {renderGroupTitle(selected)}
        </button>
      )}
    </aside>
  );
