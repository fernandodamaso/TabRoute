...
export type FlatRuleResult =
  | { kind: "representable"; draft: FlatRuleDraft }
  | { kind: "unrepresentable"; rule: Rule; reason: string };
...
export function fromRule(rule: Rule): FlatRuleResult {
  const required =
    rule.positive.kind === "all" ? rule.positive.children : [rule.positive];
  if (
    rule.positive.kind === "any" ||
    required.length === 0 ||
    required.some((node) => !isLeaf(node))
  )
    return {
      kind: "unrepresentable",
      rule,
      reason:
        "Nested or OR conditions cannot be represented by the flat editor."
    };
  if (rule.negative.some((node) => !isLeaf(node)))
    return {
      ...
      rule,
      reason:
        "Nested negative conditions cannot be represented by the flat editor."
    };
  return {
    kind: "representable",
    draft: {
      ...
      required: required as ConditionLeaf[],
      exceptions: rule.negative as ConditionLeaf[],
...
export function toRule(draft: FlatRuleDraft) {
  if (draft.required.length === 0)
    throw new Error("at least one positive condition is required");
  const rule = {
    ...
    positive:
      draft.required.length === 1
        ? draft.required[0]!
        : { kind: "all" as const, children: draft.required },
    negative: draft.exceptions,
    actions: draft.actions,
    ...(draft.duplicatePolicy === undefined
      ? {}
      : { duplicatePolicy: draft.duplicatePolicy }),
    enabled: draft.enabled,
    ...(draft.pausedUntil === undefined
      ? {}
