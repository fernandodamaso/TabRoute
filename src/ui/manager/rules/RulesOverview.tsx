...
function isPaused(value: Rule["pausedUntil"]) { return value === "restart" || (typeof value === "number" && value > Date.now()); }
export function ruleStatus(rule: Rule): Exclude<RuleFilter, "All"> { return !rule.enabled ? "Off" : isPaused(rule.pausedUntil) ? "Paused" : "Active"; }
function summary(node: ConditionNode): string {
  if (node.kind === "all" || node.kind === "any") return node.children.map(summary).join(node.kind === "all" ? " AND " : " OR ");
...
export function RulesOverview({ configuration, command, onEdit, onCreate }: {
  ...
  return <div className="rules-overview">
    <div className="rules-overview-heading"><div><p className="manager-eyebrow">AUTOMATION</p><h1>Rules</h1></div><button type="button" className="primary-button" onClick={onCreate}>Add rule</button></div>
    <div className="rules-filters" role="group" aria-label="Rule filters">{(["All", "Active", "Paused", "Off"] as const).map((value) => <button key={value} type="button" aria-pressed={filter === value} onClick={() => setFilter(value)}>{value}</button>)}</div>
    <p className="rules-count">{filtered.length} {filtered.length === 1 ? "rule" : "rules"}</p>
    <div className="rules-list">{filtered.map((rule) => {
      const target = configuration.groups.find((group) => group.id === rule.targetGroupId);
      const status = ruleStatus(rule);
      return <article key={rule.id} className="rule-row" aria-label={`Rule ${rule.id.slice(0, 8)}`}>
        <div className="rule-row-main"><div><strong>{summary(rule.positive)}</strong><p><span>Priority {rule.priority}</span><span> · </span><span>{target ? renderGroupTitle(target) : "Missing group"}</span></p></div><span className={`rule-status rule-status-${status.toLowerCase()}`}>{status}</span></div>
