...
const LICENSE = "CC BY 4.0 documentation; Apache 2.0 code samples, as stated by the source page.";
...
async function readManifest(rootDir: string): Promise<SourceManifest> {
  ...
  const parsed = JSON.parse(await readFile(manifestPath, "utf8")) as SourceManifest;
  if (!Array.isArray(parsed.sources) || parsed.sources.length === 0) {
    throw new Error("Chrome source manifest has no sources");
  }
  for (const source of parsed.sources) {
    if (!source.id || !source.file || !source.url || !source.purpose) {
      throw new Error(`Invalid Chrome source manifest entry: ${source.id ?? "unknown"}`);
    }
    const parsedUrl = new URL(source.url);
    if (parsedUrl.protocol !== "https:" || parsedUrl.hostname !== "developer.chrome.com") {
...
function articleText(html: string) {
  ...
  (article.length > 0 ? article : document("body")).find("script,style,noscript").remove();
  const text = (article.length > 0 ? article : document("body")).text().replace(/\s+/g, " ").trim();
  if (!text) throw new Error("Chrome source page contains no readable article text");
  return text;
}

function renderSnapshot(source: RefreshSource, html: string) {
  const document = load(html);
  const title = document("h1").first().text().replace(/\s+/g, " ").trim() || source.id;
  return [
...
async function validateStagedSet(rootDir: string, stagedVendor: string, sources: RefreshSource[]) {
  for (const source of sources) {
    const snapshot = await readFile(join(stagedVendor, source.file.replace(/^vendor[\\/]/, "")), "utf8");
    if (!snapshot.includes(`Source URL: ${source.url}`) || !snapshot.includes(`Source SHA-256: ${source.sha256}`)) {
...
async function swapDirectory(rootDir: string, stagedVendor: string, manifest: SourceManifest) {
  ...
  try {
    ...
    try { await rename(backupVendor, vendor); } catch { /* rollback is best effort */ }
    try { await rename(backupManifest, manifestPath); } catch { /* rollback is best effort */ }
    throw error;
  } finally {
...
export async function updateChromeDocs(input: {
  ...
  const stagedVendor = join(rootDir, `docs/chrome-reference/vendor.staged-${process.pid}-${Date.now()}`);
  ...
  try {
    for (const source of manifest.sources) {
      ...
      if (response.status !== 200) throw new Error(`${source.id}: HTTP ${response.status}`);
      const finalUrl = response.url || source.url;
      if (new URL(finalUrl).hostname !== "developer.chrome.com") {
        throw new Error(`${source.id}: final URL is outside developer.chrome.com`);
      }
      const bytes = new Uint8Array(await response.arrayBuffer());
      const refreshedSource: RefreshSource = { ...source, retrievedAtUtc, sha256: sha256(bytes) };
      ...
      await writeFile(outputPath, renderSnapshot(refreshedSource, new TextDecoder().decode(bytes)));
    }
    const readme = await readFile(join(rootDir, "docs/chrome-reference/vendor/README.md"), "utf8").catch(() => "# Official source snapshots\n");
    ...
    return { updatedIds: refreshed.map((source) => source.id), sources: refreshed };
  } catch (error) {
...
export async function validateChromeDocs(rootDir: string) {
  ...
  for (const source of manifest.sources) {
    if (!source.retrievedAtUtc || !/^\d{4}-\d{2}-\d{2}T.*Z$/.test(source.retrievedAtUtc)) {
    ...
    const snapshot = await readFile(join(rootDir, "docs/chrome-reference", source.file), "utf8");
    if (!snapshot.includes(`Source URL: ${source.url}`) || !snapshot.includes(`Retrieved at (UTC): ${source.retrievedAtUtc}`) || !snapshot.includes(`Source SHA-256: ${source.sha256}`)) {
      throw new Error(`${source.id}: snapshot metadata is incomplete`);
    }
  }
  return { valid: true, checkedIds: manifest.sources.map((source) => source.id) };
}
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(fileURLToPath(import.meta.url))) {
  ...
  if (process.argv.includes("--check")) {
    ...
    const result = await updateChromeDocs({ rootDir, fetchImpl: fetch, now: () => new Date() });
