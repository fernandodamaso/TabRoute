...
function isOwner(value: unknown): value is LockOwner {
  ...
  return (
    ...
    typeof owner.heartbeat === "number" &&
    Number.isFinite(owner.heartbeat) &&
    typeof owner.token === "string" &&
    (owner.mtimeHeartbeat === undefined || owner.mtimeHeartbeat === true)
  );
}
...
async function readSnapshot(
  ...
  try {
    ...
    try {
      ...
      /* malformed owner is recovered only by the conservative timeout */
    }
...
async function stale(
  ...
  const heartbeat = snapshot.owner.mtimeHeartbeat
    ? snapshot.mtimeMs
    : snapshot.owner.heartbeat;
...
export function createCrossProcessLock(
  ...
  const failure = () => {
    const error = new Error(
      supplied.failureCode ?? "WORKBENCH_CAPACITY"
    ) as Error & { code: string; phase: string };
    error.code = supplied.failureCode ?? "WORKBENCH_CAPACITY";
    error.phase =
      supplied.failureCode === "WORKBENCH_ARTIFACT_LIMIT"
        ? "artifact"
        : "capacity";
    return error;
  };
      ...
      try {
        ...
        try {
          ...
          if (entries.length === 0) {
            ...
            if (now() - stat.mtimeMs > TEN_MINUTES_MS)
              await fs.rmdir(guardPath).catch(() => undefined);
          } else if (
            entries.length === 1 &&
            entries[0]?.startsWith("owner-") &&
            entries[0].endsWith(".json")
          ) {
          ...
          /* another process owns or is releasing the guard */
        }
    ...
    async acquire() {
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        await withGuard(async () => {
          ...
          if (
            !current ||
            !sameFile(observed, current) ||
            current.owner?.token !== observed.owner?.token ||
            !(await stale(current, now, isPidAlive))
          )
            return;
          ...
          heartbeatTask = (async () => {
            try {
              ...
              if (
                ...
                current.dev !== heldSnapshot.dev ||
                current.ino !== heldSnapshot.ino
              )
                return;
              ...
              /* replacement owners and disappearing paths are never modified */
            }
          })().finally(() => {
            ...
            try {
              await withGuard(async () => {
                ...
                if (
                  ...
                  observed.dev !== heldStat.dev ||
                  observed.ino !== heldStat.ino
                )
                  return;
