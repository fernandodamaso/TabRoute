...
export function resolveRunPaths(
  worktreePath: string,
  runId: string,
  profilePath = path.join(os.tmpdir(), "tabroute-workbench", runId)
): RunPaths {
...
export function isOwnedProfilePath(
  ...
  if (
    ...
    candidate === worktree ||
    (worktree && candidate.startsWith(`${worktree}${path.sep}`))
  )
    return false;
  const name = path.basename(candidate);
  return (
    path.dirname(candidate) === root &&
    (name === runId || name === `profile-${runId}`)
  );
}

export function assertOwnedProfilePath(
  profilePath: string | undefined,
  profileRoot: string,
  runId: string,
  worktreePath?: string
): string {
  if (!isOwnedProfilePath(profilePath, profileRoot, runId, worktreePath))
    throw new Error("WORKBENCH_PATH_BOUNDARY");
...
export function resolveArtifactPath(
  ...
  return assertPathWithin(
    artifactRoot,
    path.resolve(artifactRoot, relativePath)
  );
}
