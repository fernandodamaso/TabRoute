...
function validPersistentTab(value: unknown): boolean {
  return validRecord(value) && value.schemaVersion === 1 && validString(value.id) && validString(value.managedGroupId) && validString(value.canonicalUrl) && Array.isArray(value.acceptedPatterns) && value.acceptedPatterns.every(validString) && validFiniteNumber(value.order) && Number.isInteger(value.order) && validFiniteNumber(value.createdAt) && validFiniteNumber(value.updatedAt) && hasOnlyKeys(value, ["schemaVersion", "id", "managedGroupId", "canonicalUrl", "acceptedPatterns", "order", "createdAt", "updatedAt"]);
}
function validConfiguration(value: unknown): boolean {
  if (!validRecord(value) || value.schemaVersion !== 1 || !validString(value.fallbackGroupId) || typeof value.automationEnabled !== "boolean" || !Array.isArray(value.groups) || !value.groups.every(validManagedGroup) || !Array.isArray(value.rules) || !value.rules.every((rule) => validRule(rule)) || !Array.isArray(value.persistentTabs) || !value.persistentTabs.every(validPersistentTab) || !Array.isArray(value.templates) || value.templates.length !== 0 || !validRecord(value.duplicateSettings) || !hasOnlyKeys(value.duplicateSettings, ["globalPolicy", "globalExclusions", "trackingParameters"]) || !validDuplicatePolicy(value.duplicateSettings.globalPolicy) || !Array.isArray(value.duplicateSettings.globalExclusions) || !value.duplicateSettings.globalExclusions.every(validString) || !Array.isArray(value.duplicateSettings.trackingParameters) || !value.duplicateSettings.trackingParameters.every(validString) || !validFiniteNumber(value.snapshotIntervalMinutes) || value.activityLimit !== 500 || value.snapshotLimit !== 50 || value.undoTtlMs !== 30000 || !validFiniteNumber(value.createdAt) || !validFiniteNumber(value.updatedAt)) return false;
  return hasOnlyKeys(value, ["schemaVersion", "fallbackGroupId", "automationEnabled", "globalPausedUntil", "groups", "rules", "persistentTabs", "restorePersistentGroups", "duplicateSettings", "templates", "snapshotIntervalMinutes", "activityLimit", "snapshotLimit", "undoTtlMs", "createdAt", "updatedAt"]) && (value.globalPausedUntil === undefined || validFiniteNumber(value.globalPausedUntil) || value.globalPausedUntil === "restart") && (value.restorePersistentGroups === undefined || typeof value.restorePersistentGroups === "boolean");
}
...
function validManagerResponse(value: unknown): boolean {
  ...
  if (value.ok) {
    ...
    return Object.values(value.viewFixture.persistentTabsByGroup).every((fixture) => validRecord(fixture) && ["loading", "empty", "populated", "disabled", "error"].includes(fixture.state as string) && Array.isArray(fixture.tabs) && fixture.tabs.every(validString) && hasOnlyKeys(fixture, ["state", "tabs", "persistentTabRecords"]) && (fixture.persistentTabRecords === undefined || (Array.isArray(fixture.persistentTabRecords) && fixture.persistentTabRecords.every(validPersistentTab))));
  }
