...
export function isLeaseReapable(
  lease: LeaseRecord,
  now: Date,
...
export class LeaseManager {
  ...
  constructor(private readonly options: LeaseManagerOptions) {
    ...
    this.profileRoot = path.resolve(
      options.profileRoot ??
        path.join(path.dirname(this.options.artifactRoot), "profiles")
    );
    this.lock = createCrossProcessLock(path.join(this.root, ".lock"), {
      runId: "lease-manager",
      isPidAlive: this.isProcessAlive
    });
  }
  validateProfilePath(
    profilePath: string | undefined,
    runId: string
  ): profilePath is string {
    return isOwnedProfilePath(
      profilePath,
      this.profileRoot,
      runId,
  ...
  async createLease(
    record: Omit<LeaseRecord, "status"> & { status?: LeaseRecord["status"] }
  ): Promise<LeaseRecord | CapacityFailure> {
    if (!this.validateProfilePath(record.profilePath, record.runId))
      return {
        ok: false,
        status: "failed",
        ...
        error: { message: "profile path is not owned by this run" }
      };
    try {
      return await this.lock.withLock(async () => {
        ...
        if (active >= MAX_ACTIVE_LEASES)
          return {
            ...
            runId: record.runId,
            worktreePath: this.options.worktreePath,
            error: { message: "workbench active lease capacity reached" }
          };
        ...
        await writeAtomic(
          ...
          new TextEncoder().encode(JSON.stringify(lease))
        );
        return lease;
      });
    } catch (error) {
        ...
        error: {
          message:
            error instanceof Error
              ? error.message
              : "workbench capacity unavailable"
        }
      };
    }
  }
  ...
  async heartbeat(runId: string, at = this.now().toISOString()): Promise<void> {
    await this.lock.withLock(async () => {
      ...
      await writeAtomic(
        file,
        new TextEncoder().encode(JSON.stringify({ ...lease, heartbeat: at }))
      );
    });
  }
  ...
  async reapOrphans(): Promise<
    Array<AbandonedRunResult | AbandonedCleanupFailure>
  > {
    return this.lock.withLock(async () => {
      ...
      for (const lease of await this.readLeases()) {
        ...
        if (
          !validateRunResult(existing) ||
          existing.status === "abandoned" ||
          !validateStartedMetadata(existing)
        )
          continue;
        ...
        for (
          let attempt = 0;
          attempt <= CLEANUP_BACKOFF_MS.length;
          attempt += 1
        ) {
          try {
            const profile = assertOwnedProfilePath(
              lease.profilePath,
              this.profileRoot,
              lease.runId,
              this.options.worktreePath
        ...
        const base = {
          ...
          commandRecords: existing.commandRecords,
          readiness: existing.readiness,
          screenshotPaths: existing.screenshotPaths,
          assertions: existing.assertions,
          lease: abandonedLease,
          ...(typeof (
            existing as Partial<RunResult & { extensionId?: unknown }>
          ).extensionId === "string"
            ? {
                extensionId: (existing as unknown as { extensionId: string })
                  .extensionId
              }
            : {})
        };
        const updated = removed
          ...
          : {
              ...base,
              ok: false as const,
              code: "WORKBENCH_CLEANUP_FAILED" as const,
              phase: "cleanup" as const,
              cleanup: {
                profileRemoved: false as const,
                retainedPath: lease.profilePath
              },
              error: boundedError({
                message:
        ...
        await writeAtomic(
          path.join(directory, "lease.json"),
          new TextEncoder().encode(JSON.stringify(abandonedLease))
        );
  ...
  startHeartbeat(
    ...
    > = globalThis,
    beat: () => Promise<void> = () => this.heartbeat(runId)
  ): { stop(): void } {
